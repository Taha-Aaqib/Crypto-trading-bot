"""
Retrain ML Model with Balanced Classes
This fixes the 91% HOLD bias problem
"""
...
def create_labels_percentile(df: pd.DataFrame, lookahead: int = 10,
    ...
    rolling_window = 200
    upper_threshold = future_returns.rolling(
        rolling_window).quantile(buy_percentile/100)
    lower_threshold = future_returns.rolling(
        rolling_window).quantile(sell_percentile/100)
...
def main():
    print("=" * 70)
    print("🔧 RETRAINING ML MODEL WITH BALANCED CLASSES")
    print("=" * 70)
    ...
    # Get November 2025 data for training
    ...
    # Save model
    print("\n💾 Saving new balanced model...")

    model_dir = "models/saved_models/BTC_USDT"
    ...
    if os.path.exists(old_model_path):
        backup_path = os.path.join(
            model_dir, "lstm_model_backup_imbalanced.pkl")
    ...
    print("\n🧪 Now run 'python debug_ml.py' to verify predictions!")
